...
def upsample(idims, odims, stride=(2, 2), mode="deconv"):
    if mode == "deconv":
        return nn.Sequential(
            nn.ConvTranspose2d(idims, odims, stride, stride=stride, bias=False),
...
class ConvBNReLU(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride=1, eps=1e-3, momentum=0.01
    ):
        super().__init__()
    ...
    def __init__(self, idims, odims, stride=1):
        ...
        if idims == odims and stride == 1:
            self.conv2 = nn.Sequential(
                nn.Conv2d(
                    idims, 6 * idims, 3, stride=1, padding=1, groups=idims, bias=False
                ),
            ...
            self.conv2 = nn.Sequential(
                nn.Conv2d(
                    idims, 6 * idims, 3, stride=2, padding=1, groups=idims, bias=False
                ),
                nn.BatchNorm2d(6 * idims, eps=BN_EPS, momentum=BN_MOMENTUM),
                nn.Conv2d(
                    6 * idims,
...
class iRangeViewNet(nn.Module):
    ...
    def __init__(self, in_channels, init_type="kaiming_uniform"):
        ...
        self.res2 = iBottleneckBlock(
            idims=in_channels, odims=2 * in_channels, stride=(2, 2)
        )
        self.res3 = iBottleneckBlock(
            idims=2 * in_channels, odims=4 * in_channels, stride=(2, 2)
        )
            ...
            for m in self.modules():
                if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
                    if init_type == "kaiming_uniform":
                        ...
                        utils.kaiming_init(m, distribution="normal")
                    elif init_type == "xavier":
                        utils.xavier_init(m)
                    elif init_type == "caffe2_xavier":
                        utils.caffe2_xavier_init(m)
                elif isinstance(m, nn.BatchNorm2d):
                    utils.constant_init(m, 1)
