...
class DGL_ResGatedGraphEncoder(torch.nn.Module):
    def __init__(
        ...
        self.layers = torch.nn.ModuleList(
            [
                DGL_ResGatedGraphConv(
                    embedding_dim, embedding_dim, dropout, batch_norm, residual
                )
                for _ in range(n_layers)
            ]
        )
    ...
    def __init__(
        ...
        for i in range(1, n_layers + 1):
            if conv_type == "agnn_conv":
                ...
                conv = pyg.nn.GATConv(
                    embedding_dim, embedding_dim // n_heads, heads=n_heads
                )
...
class LoopDecoder(torch.nn.Module):
    def __init__(
        self, embedding_dim, n_heads=8, tanh_clipping=10.0, n_decoding_neighbors=None
    ):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.n_heads = n_heads
    ...
    def forward(self, last_node):
        ...
        if self.q_first is None:
            self.q_first = utils.make_heads(
                self.Wq_first(last_node_embedding), self.n_heads
            )
...
class PathDecoder(torch.nn.Module):
    ...
    def reset(
        ...
        self.q_source = utils.make_heads(
            self.Wq_source(source_node_embedding), self.n_heads
        )
        ...
        self.q_target = utils.make_heads(
            self.Wq_target(target_node_embedding), self.n_heads
        )
        ...
        self.q_first = utils.make_heads(
            self.Wq_first(first_node_embedding), self.n_heads
        )
        # glimpse_k.shape = glimpse_v.shape =[B, n_heads, N, key_dim]
        # logit_k.shape = [B, H, N]
        # group_ninf_mask.shape = [B, G, N]
    ...
    def forward(self, last_node):
        ...
        probs = F.softmax(score_masked.float(), dim=2, dtype=torch.float32).type_as(
            score_masked
        )
        assert (probs == probs).all(), "Probs should not contain any nans!"
        return probs
...
class AddPathDecoder(torch.nn.Module):
    ...
    def reset(self, coordinates, embeddings, group_ninf_mask, source_node, target_node):
        # embeddings.shape = [B, N, H]
        # graph_embedding.shape = [B, 1, H]
        B, N, H = embeddings.shape
        self.coordinates = coordinates
